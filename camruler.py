...
framename = "Pendeteksi Area Telur"
cv2.namedWindow(framename,flags=cv2.WINDOW_NORMAL|cv2.WINDOW_GUI_NORMAL)


# key event
key_last = 0
key_flags = {'config':False, # c kunci
             'auto':False,   # a kunci
             'thresh':False, # t kunci
             'percent':False,# p kunci
             'norms':False,  # n kunci
             'rotate':False, # r kunci
             'lock':False,   #
             }
...
while 1:
    ...
    if key_flags['config']:
        ...
        # crosshairs rotate to max distance
    ...
    # Auto Mode
        ...
        for c in contours:
            ...
            if carea <= 2000 :
                draw.add_text(frame0, f'Status: KURANG BAIK', x4, y4 + 10, center=True, top=True, color='green')
            elif 2000 <= carea <= 4000 :
                draw.add_text(frame0, f'Status: BAIK', x4, y4 + 10, center=True, top=True, color='green')
            elif 4000 <= carea <= 6000 :
                draw.add_text(frame0, f'Status: SANGAT BAIK', x4, y4 + 10, center=True, top=True, color='green')
            elif carea >= 6000 :
                draw.add_text(frame0, f'Status: SANGAT BAIK', x4, y4 + 10, center=True, top=True, color='green')
            if alen:
